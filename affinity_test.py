...
if __name__ == '__main__':
    ...
    for pdbname in complexdict.keys():
        ...
        #     x = torch.tensor(node_features, dtype=torch.float).to(args.device)
        #     edge_index=torch.tensor(edge_index,dtype=torch.long).t().contiguous().to(args.device)
    ...
    for i in range(5):
        ...
        net.load_state_dict(torch.load(f"/root/xukeyu/affinity_model{i}_dim{args.dim}_foldx.pt"))

        dataset=MyGCNDataset(featureList)
        dataloader=DataLoader(dataset,batch_size=args.batch_size,shuffle=True)
        criterion = torch.nn.MSELoss()
        
        test_prelist, test_truelist,test_loss = gcn_predict(net,dataloader,criterion,args.device,i,0)
    ...
    print('rmse :   '+str(best_rmse/5))
    print('mae  :   '+str(best_mae/5))